...
class mainbot:
    ...
    async def handle(message: types.Message):
        check = await user_checkin(message)
        ...
        if check:
            ...
            videosaver.video_saver(r1, filename)

            await message.answer_video(open(os.path.join("{}.mp4".format(str(filename))), 'rb'))
